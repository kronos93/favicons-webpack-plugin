...
module.exports.compileTemplate = function compileTemplate(options, context, compilation) {
  ...
  var singleEntryPlugin = new SingleEntryPlugin(context, '!!' + require.resolve('./favicons.js') + '?' +
    JSON.stringify({
      ...
      appShortName: options.config.appShortName,
      appDescription: options.config.appDescription,
      developerName: options.config.developerName,
      developerURL: options.config.developerURL,
      dir: options.config.dir,
      lang: options.config.lang,
      background: options.config.background,
      theme_color: '#DFD',//options.config.theme_color,
      themeColor: '#DFD',//options.config.theme_color,
      theme_color: "#fff",
      display: options.config.display,
      appleStatusBarStyle: options.config.appleStatusBarStyle,
      orientation: options.config.orientation,
      start_url: options.config.start_url,
      scope: options.config.scope,
      version: options.config.version,
      logging: options.config.logging,
      icons: options.config.icons,
    }) + '!' + options.logo);
